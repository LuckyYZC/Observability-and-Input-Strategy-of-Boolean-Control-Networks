...
\begin{itemize}
\item When $t_k=t+1$, for any distinct states $\mathsf{s}(t)$, $\mathsf{s}'(t) \in \mathsf{S}(t)$, $H^{[t,t_k]}(\mathsf{s}'(t),\mathsf{I}[t,t_k])\neq H^{[t,t_k]}(\mathsf{s}(t), \mathsf{I}[t,t_k])$. Then we have for $\mathsf{S}(t)$,
 there exists an input $\mathsf{i}(t)=\mathsf{I}[t,t_k]$ such that
 \begin{itemize}
