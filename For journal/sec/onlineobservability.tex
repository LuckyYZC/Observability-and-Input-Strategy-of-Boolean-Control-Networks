...
%\end{split}
%\end{equation*}
where $\mathsf{S}\in 2^{\mathcal{S}_\BB}$, $\mathsf{i} \in (\Delta_L\cup\varepsilon)$, $\mathsf{o} \in (\mathcal{O}_\BB\cup\varepsilon)$, and $\varepsilon$ presents empty output.
%\tl{check whether this is what you want!?}\gs{It is ok.}
\end{definition}

 Then the set $\mathsf{S}(t)$ of possible valuations of $\mathsf{s}(t)$ of a \BCN\ $\BB$ is defined as follows. 
...
For a non-empty set $\mathsf{S}\subseteq\mathcal{S}_\BB$
 ...
 \item  if $|\mathsf{S}|>1$ and there is an input $\mathsf{i} \in \mathcal{I}_\BB$ such that % $\Ks(\mathsf{S})=\min(\{\max\{\Ks(\Ded(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j}))| \Ded(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j})\ne \emptyset, \mathsf{o}^{j}\in\Delta_Q\}||\Ded(\mathsf{S},\mathsf{i}^{x},\varepsilon)|=|\mathsf{S}|, \mathsf{i}^{x} \in \Delta_M\})+1$ 
 \begin{itemize}
 \item  $|\Ded(\mathsf{S},\mathsf{i},\varepsilon)|=|\mathsf{S}|$, and 
 \item   for every $\mathsf{o} \in \mathcal{O}_\BB$, $\Ded(\mathsf{S},\mathsf{i},\mathsf{o})\ne \emptyset$ implies $\Ks(\Ded(\mathsf{S},\mathsf{i},\mathsf{o}))< k$;
 \end{itemize} 
 and there is not any $\mathsf{i}' \in \mathcal{I}_\BB$ which satisfies that
  \begin{itemize}
 \item  $|\Ded(\mathsf{S},\mathsf{i}',\varepsilon)|=|\mathsf{S}|$, and 
...
As mentioned in the {\em Example \ref{exa:8}}, $|\mathsf{S}(0)|=3$, and there exists $\delta_{4}^3$ such that 
 ...
 \item   and for each $\mathsf{o}\in \mathcal{O}_\BB$
  \begin{itemize}
...
\begin{equation*}
\begin{split}
&\Ri(\mathsf{S})=\{\mathsf{i}\in \mathcal{I}_\BB|\  |\Ded\left(\mathsf{S},\mathsf{i},\varepsilon\right)|=|\mathsf{S}|, \\
& \forall \mathsf{o} \in \mathcal{O}_\BB, \Ded(\mathsf{S},\mathsf{i},\mathsf{o})\ne \emptyset \rightarrow \Ks(\Ded(\mathsf{S},\mathsf{i},\mathsf{o}))\ne \infty\}
\end{split}
\end{equation*}
\end{definition}
\begin{lemma}
...
%\tl{(12)why $\epsilon$ is in the def? for which purpose?}
 A \BCN\ is observable
if for every $\mathsf{o}(0)\in  \mathcal{O}_\BB$, $\Ded(\mathcal{S}_\BB,\varepsilon, \mathsf{o}(0))\ne\emptyset$ implies $\Ks(\Ded\left(\mathcal{S}_\BB,\varepsilon,\mathsf{o}(0)\right))\ne\infty$.
\end{definition}
