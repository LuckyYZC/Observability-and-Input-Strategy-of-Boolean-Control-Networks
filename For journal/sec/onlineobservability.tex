...
As mentioned in the introduction, to determine $\mathsf{s}(0)$ %by taking the determining procedure once, we only need to complete the following procedure.
...
Based on this procedure, we propose the online observability, viz., a \BCN\ is observable if the procedure to determine $\mathsf{s}(0)$ terminates for every \State$(0)\in \Delta_N$.  %\tl{do you mean that the procedure terminates?} \gs{Yes} 
...
For a non-empty $\mathsf{S}$
 ...
 \item  if $|\mathsf{S}|>1$ and there is an input $\mathsf{i}^{x} \in \Delta_M$ such that % $\Ks(\mathsf{S})=\min(\{\max\{\Ks(\Ded(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j}))| \Ded(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j})\ne \emptyset, \mathsf{o}^{j}\in\Delta_Q\}||\Ded(\mathsf{S},\mathsf{i}^{x},\varepsilon)|=|\mathsf{S}|, \mathsf{i}^{x} \in \Delta_M\})+1$ 
 \begin{itemize}
...
As mentioned in the {\em Example \ref{exa:8}}, $|\mathsf{S}(0)|=3$, as there exists $\delta_{4}^3$ such that 
 ...
 \end{itemize}
 \end{itemize}
$\Ks(\mathsf{S}(0))=1$, $\mathsf{s}(0)$ can be determined at time $1$.
\end{example}  
...
\begin{figure}[thpb]
      \centering
      \framebox{\parbox{3in}{\centerline{\includegraphics[scale=0.28]{figures/Fig11.png}} }} 
...
In conclusion, the online observability addresses the necessary and sufficient condition of determining $\mathsf{s}(0)$ by taking the determining procedure once, because at every time $t$, we observe $\mathsf{o}(t)$ of \BCN\ and choose $\mathsf{i}(t)$ based on the information we have collected so far. Such that the online observability enriches the control theory of \BCNs\ and determine the initial states of some \BCNs\ which can not be determined before. 