...
For $k>0$, a set of states $\mathsf{S}(t)$ is $k$-step determinable
 iff $|\mathsf{S}(t)|>0$ and for $\mathsf{S}(t)$ there exists an input $\mathsf{i}^{i}(t) \in \Delta_M$ such that
 \begin{itemize}
...
So we have that the definition is right when $k=0$.
 
\item When $k=1$, for $\mathsf{S}(t)$ we can determine the state $\mathsf{s^{i}}(t)$ at time step $t+1$ for every $\mathsf{s}^{i}(t)\in \mathsf{S}(t)$. Thus we need to guarantee that
\begin{itemize}
\item $|\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\varepsilon\right)|=|\mathsf{S}(t)|$, only in this case we can determine the state $\mathsf{s^{i}}(t)$ at time step $t+1$ for every $\mathsf{s}^{i}(t)\in \mathsf{S}(t)$ when we can determine the corresponding state $\mathsf{s^{i}}(t+1)$ at time step $t+1$ for every $\mathsf{s}^{i}(t+1)\in \Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\varepsilon\right)$;
\item And we can determine the state $\mathsf{s^{i}}(t+1)$ at time step $t+1$ for every $\mathsf{s}^{i}(t+1)\in \Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\varepsilon\right)$. To guarantee this we have that $|\mathsf{S}(t+1)|=1$, then we have that for each $\mathsf{o}^{j}(t+1)\in \Delta_Q$ such that $|\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)|\ne0$, there exists a ${k'}=0<k=1$ such that $\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)$ is $k'$-step determinable.
 \end{itemize}
So we have the definition is right when $k=1$.

\item  If for $k=0,\ldots k=p$ the definition is right. When $k=p+1$, for $\mathsf{S}(t)$ we can determine the state $\mathsf{s^{i}}(t)$ at time step $t+p+1$ for every $\mathsf{s}^{i}(t)\in \mathsf{S}(t)$. Thus we need to guarantee that
\begin{itemize}
\item $|\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\varepsilon\right)|=|\mathsf{S}(t)|$, only in this case we can determine the state $\mathsf{s^{i}}(t)$ at time step $t+p+1$ for every $\mathsf{s}^{i}(t)\in \mathsf{S}(t)$ when we can determine the corresponding state $\mathsf{s^{i}}(t+1)$ at time step $t+p+1$ for every $\mathsf{s}^{i}(t+1)\in \Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\varepsilon\right)$;
\item And we can determine the state $\mathsf{s^{i}}(t+1)$ at time step $t+p+1$ for every $\mathsf{s}^{i}(t+1)\in \Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\varepsilon\right)$. To guarantee this we have for each $\mathsf{o}^{j}(t+1)\in \Delta_Q$ such that $|\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)|\neq 0$, there exists a ${k'}<k$ such that $\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)$ is $k'$-step determinable.
 \end{itemize}
So we have the definition is right when $k=p+1$.

 \end{itemize}
 
Therefore, we have the definition is right for every $k\in \mathbb{N}$.
...
\begin{lemma}
 If a set of possible states $\mathsf{S}(t)$ is $k_1$-step deterministic and $k_1\le k_2$, then $\mathsf{S}(t)$ is $k_2$-step determinable. %But if the states set $S$ is $k_1$-step deterministic and $k_1> k_2$, we can not deduce that $S$ is $k_2$-step deterministic. 
  \label{lemm:2}
\end{lemma}

\begin{proof}
 A set of possible states $\mathsf{S}(t)$ is $k_1$-step deterministic and the $k_1\le k_2$. 
 ...
 \item  for each $o_j$ in $\Delta_Q$ such that $|\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)|=1\neq 0$ that $\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)$ is $k'$-step deterministic where ${k'}=0=k_1\le k_2$.
 \end{itemize}
 ...
 \begin{itemize}
 \item  $|\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\varepsilon\right)|=|\mathsf{S}(t)|$, and 
 \item  for each $o_j$ in $\Delta_Q$ such that $|\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)|\neq 0$ and $\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)$ is $k'$-step deterministic and ${k'}<k_1\le k_2$.
 \end{itemize}
 Therefore the set of states $\mathsf{S}(t)$ is $k_2$-step deterministic.

 So we have the {\em Lemma \ref{lemm:2}} is right.
\end{proof}

However if the states set $\mathsf{S}(t)$ is $k_1$-step deterministic and $k_1> k_2$. What is more, for this set of states $\mathsf{S}(t)$ there is no such $i_p$ in $\Delta_M$ such that
 \begin{itemize}
 \item  $|\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\varepsilon\right)|=|\mathsf{S}(t)|$, and 
 \item  for each $o_j$ in $\Delta_Q$ such that $|\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)|\neq 0$ and $\Ded\left(\mathsf{S}(t),\mathsf{i}^{i}(t),\mathsf{o}^{j}(t+1)\right)$ is $k'$-step deterministic with  ${k'}<k_2<k_1$.
 \end{itemize}
 Then the set of states $\mathsf{S}(t)$ is not $k_2$-step deterministic.

From the {\em Lemma \ref{lemm:2}}, we know that the exists the infimum of $k$ for the $k$-step determinability of a set of states $\mathsf{S}(t)$, which indicate the shortest path to definitely determine the state of a \BCN\ by this possible states set $\mathsf{S}(t)$.
...
\begin{lemma}
 If a set of states $\mathsf{S}^{j}(t)$ is $k$-step determinable and $\mathsf{S}^{i}(t)\subseteq \mathsf{S}^{j}(t)$, then $\mathsf{S}^{j}(t)$ is $k$-step determinable.
  \label{lemm:4}
\end{lemma}
\begin{proof}
A set of states $\mathsf{S}^{j}(t)$ is $k$-step deterministic and $\mathsf{S}^{i}(t)\subseteq \mathsf{S}^{j}(t)$. Therefore we have $k>0$ from this precondition.
