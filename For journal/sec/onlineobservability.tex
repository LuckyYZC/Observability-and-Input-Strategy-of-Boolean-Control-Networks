...
\ly{In the existing third observability, there has to exist an input sequence that determines its initial state $\mathsf{s}(0)$ if a \BCN\ satisfies the observability. }
But we can derive the set of possible initial states $\mathsf{S}(0)$ by initial output $\mathsf{o}(0)$ we observe at time step $0$. For every $\mathsf{S}(0)$ we derived, if we can find an input sequence to determine initial state ($\mathsf{s}(0)\in \mathsf{S}(0)$), we can determine the initial state too. The corresponding input sequences of the sets of possible initial states can be different. 
In this case, the requirements for the \BCN\ to determine the initial state would be easier to meet. Furthermore, if we utilize the sets of possible initial states $\mathsf{S}(0)$ and $\mathsf{S}(1)$ derived at time step $0$ and $1$ to find the input sequence to determine the initial state, the requirements for \BCNs\ would be further easier to meet. 

According to this law, in the online observability we find input sequence one by one according to the $\mathsf{S}(t)$ we derive at every time step, such that the requirements would be easiest to meet. Thus, a \BCN\ satisfies the online observability iff its initial state $\mathsf{s}(0)$ can be determined in real time for every $\mathsf{s}(0) \in \Delta_N$.
