...
\label{sec:online}
In this section we propose the online observability to solve the problem mentioned before, and we will introduce its related information in detail. We give the informal definition of it at first. 

%\begin{definition}
	A \BCN\ satisfies the online observability iff its initial state $s_0$ can be determined in real time for every $s_0 \in \Delta_N$. %In the online observability we determine the state of \BCN\ by observing its output sequence and then deriving and deciding the input sequence at every time step. What is more, this process can be accomplished in finite time steps.
...
After introducing the properties of the online observability, we present the process of determining the initial state of \BCNs\ by online observability briefly. We observe the output of \BCNs\ at every time setp $k$ in the process of determining the initial state of \BCNs. Then, we can infer the possible values of state-nodes through the output and denote them by possible states set $S_k$. %Then as we can know the possible states set
With the set of possible states we can derive the set of possible inputs $I_k$, such that for every $i\in I_k$ we have that $s_i, s_j$ will not turn into the same state after being affected by the input $i_k$ i.e., $Ls_i i_k\neq Ls_j i_k$ for any distinct $s_i, s_j\in S_k$. This principle will be shown in the equation (\ref{equ:12}). And then, we can decide the input by choosing it from the $I_k$ that this input will help us determine the initial state in real time. The way to decide the input will be presented in subsequent subsections. The cardinal number $|S_k|$ of possible states set does not change or decrease in this process. This principle will be shown in the equations (\ref{equ:9}) and (\ref{equ:10}). We repeat the above process, if the cardinal number of possible states set turn into be $1$ then we can determine the state and the initial state of {\em BCN}. 

 In the left of this section, firstly we define derivation function to describe the derivation process of the input and state of \BCNs\ at every time step. Secondly we present the definition of $k$-step determinability to help us decide the input. Thirdly, we give the formal definition of the online observability of \BCNs. Finally, we compare the online observability with four existing observability.
