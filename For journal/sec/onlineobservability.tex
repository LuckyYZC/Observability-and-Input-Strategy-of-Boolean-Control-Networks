...
%-------------------------------------
To formally define the online observability, we start with he derivation function $\Ded( \mathsf{S},  \mathsf{i},  \mathsf{o})$ which formalize how to derive $\mathsf{S}(t)$ from $\mathsf{o}(t)$, $\mathsf{i}(t-1)$, $\mathsf{S}(t-1)$ and the updating rules of \BCN.  %The definition of it is as follows.
...
\end{definition}
 ...
 \begin{definition}[$\mathsf{S}(t)$] Given $\mathsf{I}[k]=\mathsf{i}(0)\ldots\mathsf{i}(k-1)$, $\mathsf{o}(0)$ and $(HF)^k_{\mathsf{s}(0)}(\mathsf{I}[k])=\mathsf{o}(1)\ldots\mathsf{o}(k)$, %for every $t\le k$, 
	\[\mathsf{S}(t)=\left\{
\begin{array}{rcl}
...
Therefore we have this \BCN\ is the online observable, we can determine the initial state of it. And this \BCN\ is controllable, thus it is identifiable.  %Thus with the online observability we can research the \BCNs\ better.
\label{exa:10}
\end{example}  
...
From the {\em Lemma \ref{lemm:3}}, {\em Lemma \ref{lemm:4}} and the implication relations between the existing four observability which mentioned in the {\em Section \ref{sec:pre}}, we have the relations among all five types of observability is shown in Fig.~\ref{fig:7}.
