...
We write $\xi(\mathsf{i}, \mathsf{s}):= \left\{
\begin{array}{rcl}
...
For a non-empty set $\mathsf{S}\subseteq\Delta_M$
 ...
 \item  $\Ks(\Ded(\mathsf{S},\mathsf{i},\mathsf{o}))< k$ for every non-empty $\Ded(\mathsf{S},\mathsf{i},\mathsf{o})$, where $\mathsf{o} \in \Delta_N$, 
 \end{itemize} 
 and there is not any $\mathsf{i}' \in \Delta_L$ satisfies that
  \begin{itemize}
 \item  $|\Ded(\mathsf{S},\mathsf{i}',\varepsilon)|=|\mathsf{S}|$, and 
 \item  $\Ks(\Ded(\mathsf{S},\mathsf{i'},\mathsf{o}))< k-1$ for every non-empty $\Ded(\mathsf{S},\mathsf{i}',\mathsf{o})$, where $\mathsf{o} \in \Delta_N$, 
 \end{itemize} 
...
%From the Step 2, we have for every $ k=1,\ldots, t$, for every $\mathsf{s}'(k)\in \mathsf{S}(k)$, there is only one corresponding $\mathsf{s}'(k-1)\in \mathsf{S}(k-1)$ such that the $\mathsf{s}'(k)$ is determined by $\mathsf{s}'(k-1)$ and $\mathsf{i}(k-1)$. Thus %once $|\mathsf{S}|=1$, we can determine $\mathsf{s}(t)$, and then determine $\mathsf{s}(t-1),\dots,\mathsf{s}(0)$.
 ...
 From the {\em Example \ref{exa:6}}  and {\em Example \ref{exa:10}}, we have the \BCN\ mentioned in {\em Example \ref{exa:2}} does satisfies the {\bf Type-III} observability but its initial state $\mathsf{s}(0)$  can be determined by above procedures without resetting. 
