...
As mentioned in the introduction, when $\mathsf{s}(0)$ cannot be reset, we can determine $\mathsf{s}(0)$ %by taking the determining procedure once, we only need to complete the following procedure.
...
\item[Step 2] With $\mathsf{S}(t)$ we can derive the set of possible inputs $\{\mathsf{i}^i(t),\ldots,\mathsf{i}^k(t)\}$, such that for every $\mathsf{i}(t)\in \{\mathsf{i}^i(t),\ldots,\mathsf{i}^k(t)\}$, for any two distinct $\mathsf{s}(t)$, $\mathsf{s}'(t) \in \mathsf{S}(t)$, $f(\mathsf{s}(t), \mathsf{i}(t))\neq f(\mathsf{s}'(t),\mathsf{i}(t))$. And then we choose $\mathsf{i}(t)$ from $\{\mathsf{i}^i(t),\ldots,\mathsf{i}^k(t)\}$.
\item[Step 3] Based on $\mathsf{s}(t+1)= f({\mathsf{i}(t)},{\mathsf{s}(t)})$, $\mathsf{S}(t+1)$ is preliminarily derived, and we need $\mathsf{o}(t+1)$ to further determine $\mathsf{S}(t+1)$. 
\end{description}

Clearly, $|\mathsf{S}(t+1)|\le|\mathsf{S}(t)|$, and we can determine $\mathsf{s}(t)$  when $|\mathsf{S}(t)|$ becomes $1$. Because for every $\mathsf{s}(t)\in $ $\mathsf{S}(t)$ there is exact one corresponding $\mathsf{s}(t-1)\in $ $\mathsf{S}(t-1)$ that $f(\mathsf{i}(t-1),\mathsf{s}(t-1))=\mathsf{s}(t)$, we can determine $\mathsf{s}(t-1)$, $\mathsf{s}(t-2)$, \ldots, $\mathsf{s}(0)$.
...
Based on this procedure, we propose the online observability, viz., a \BCN\ is observable if the procedure to determine $\mathsf{s}(0)$ terminates for every \State$(0)\in \Delta_M$.  %\tl{do you mean that the procedure terminates?} \gs{Yes} 
...
%-------------------------------------
We start with he derivation function $\Ded( \mathsf{S},  \mathsf{i},  \mathsf{o})$ which formalize how to derive $\mathsf{S}(t)$ from $\mathsf{o}(t)$, $\mathsf{i}(t-1)$, $\mathsf{S}(t-1)$ and the updating rules of \BCN.  %The definition of it is as follows.
...
%\end{split}
%\end{equation*}
where $\mathsf{S}\in 2^{\Delta_M}$, $\mathsf{i} \in (\Delta_L\cup\varepsilon)$, $\mathsf{o} \in (\Delta_N\cup\varepsilon)$, $\varepsilon$ presents empty output.
%\tl{check whether this is what you want!?}\gs{It is ok.}
\end{definition}

 Then we have $\mathsf{S}(t)$ is defined as follows. 
 \begin{definition}[$\mathsf{S}(t)$] Given $\mathsf{i}(0)\mathsf{i}(1)\ldots\mathsf{i}(k-1)$ and $\mathsf{o}(0)\mathsf{o}(1)\ldots\mathsf{o}(k)$, %for every $t\le k$, 
	\[\mathsf{S}(t)=\left\{
\begin{array}{rcl}
...
As the next step, we introduce a predicate $\Ks(\mathsf{S}(t))$ for $\mathsf{S}(t)$ to depict we can determine the $\mathsf{s}(t)$ at time $(t+\Ks(\mathsf{S}(t))$ for every $\mathsf{s}(t)\in \mathsf{S}(t)$. 
...
For a non-empty $\mathsf{S}$
 ...
 \item  if $|\mathsf{S}|>1$ and there is an input $\mathsf{i} \in \Delta_L$ such that % $\Ks(\mathsf{S})=\min(\{\max\{\Ks(\Ded(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j}))| \Ded(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j})\ne \emptyset, \mathsf{o}^{j}\in\Delta_Q\}||\Ded(\mathsf{S},\mathsf{i}^{x},\varepsilon)|=|\mathsf{S}|, \mathsf{i}^{x} \in \Delta_M\})+1$ 
 \begin{itemize}
...
Therefore we have this \BCN\ is the online observable, we can determine the initial state of it. Comparing with {\em Example \ref{exa:6}}, the online observability can be used to determine the initial states of some \BCNs\ which can not be determined by {\bf Type-III} observability when their initial state cannot be reset. And we have this \BCN\ is controllable, thus it is identifiable too.  %Thus with the online observability we can research the \BCNs\ better.
\label{exa:10}
\end{example}  
...
In conclusion, the online observability addresses the necessary and sufficient condition of determining $\mathsf{s}(0)$ when the $\mathsf{s}(0)$ cannot be reset. Because at every time $t$, we observe $\mathsf{o}(t)$ of \BCN\ and choose $\mathsf{i}(t)$ based on the information we have collected so far. To identify the updating rules of a \BCN\  from its input-output data requires its $\mathsf{s}(0)$ can be determined  when the $\mathsf{s}(0)$ can not be reset. Thus the online observability could help us further researching the identification problem of \BCNs. 