...
If the possible states set $\mathsf{S}=\{\delta_{16}^0$, $\delta_{16}^1$, $\delta_{16}^2\}$ and we input $\delta_4^0$ observe $\delta_4^2$, then we can deduce that the possible new state can be $\delta_{16}^{9}$ or  $\delta_{16}^{10}$.
 \label{exa:8}
 \end{example}   
...
As the derivation function can only describe the process of deriving the set possible states $\mathsf{S}(t)$ of \BCNs\ at every time step. But we need to know how to choose $\mathsf{i}(t)$. Thus we propose the $k$-step determinability for $\mathsf{S}(t)$ to depict whether we can determine the $\mathsf{s}(t)$ in $k$ time steps for every $\mathsf{s}(t)\in \mathsf{S}(t)$. With this notion, we can know how to derive the $\mathsf{i}(t)$ by $\mathsf{S}(t)$ in the process of determining $\mathsf{s}(0)$. 
\begin{definition}[$k$-Step Determinability] 
The $k$-step determinability of $\mathsf{S}$ is recursively defined as follows:
 \begin{itemize}
 \item When $k=0$, a set of possible states $\mathsf{S}$ is $0$-step determinable if $|\mathsf{S}|=1$. 
 \item When $k>0$, a set of states $\mathsf{S}$ is $k$-step determinable
 if $|\mathsf{S}|>0$ and for $\mathsf{S}$ there exists an input $\mathsf{i}^{i} \in \Delta_M$ such that
 \begin{itemize}
 \item  $|\Ded\left(\mathsf{S},\mathsf{i}^{i},\varepsilon\right)|=|\mathsf{S}|$, and 
 \item  for each \[\mathsf{o}^{j}(t+1)\in \Delta_Q\] such that \[|\Ded\left(\mathsf{S},\mathsf{i}^{i},\mathsf{o}^{j}\right)|\neq 0,\] there exists a ${k'}\le 
(k-1)$ such that $\Ded\left(\mathsf{S},\mathsf{i}^{i},\mathsf{o}^{j}\right)$ is $k'$-step determinable.
 \end{itemize}
 And the $\mathsf{i}^{i}$ is the input which makes $\mathsf{S}$ $k$-step determinable.
 \end{itemize}
\end{definition}
...
In the \BCN\ mentioned in {\em Example \ref{exa:2}}. For the set of possilble states $\Ded\left(\Delta_N,\varepsilon,\delta_4^0\right)=\{\delta_{16}^0,\delta_{16}^1,\delta_{16}^2\}$, the cardinality number of this set $|\{\delta_{16}^0,\delta_{16}^1,\delta_{16}^2\}|=3>1$, and for this set there exists $\delta_{4}^3$ such that 
 ...
 \item   and for each $\mathsf{o}^{j}\in \Delta_Q$
  \begin{itemize}
  \item   $|\Ded\left(\{\delta_{16}^0,\delta_{16}^1,\delta_{16}^2\},\delta_{4}^3,\delta_{4}^0\right)|=|\{\delta_{16}^0\}|=1$, that the $\{\delta_{16}^0\}$ is $0$-step determinable;
 \item  $|\Ded\left(\{\delta_{16}^0,\delta_{16}^1,\delta_{16}^2\},\delta_{4}^3,\delta_{4}^3\right)|=|\{\delta_{16}^{13}\}|=1$, that the $\{\delta_{16}^{13}\}$ is $0$-step determable;
  \item  $|\Ded\left(\{\delta_{16}^0,\delta_{16}^1,\delta_{16}^2\},\delta_{4}^3,\delta_{4}^1\right)|=|\{\delta_{16}^{6}\}|=1$, that the $\{\delta_{16}^{6}\}$ is $0$-step determinable.
 \end{itemize}
 \end{itemize}
 Therefore the set of states $\{\delta_{16}^0,\delta_{16}^1,\delta_{16}^2\}$ is $1$-step determinable.
\label{exa:9}
\end{example}  
...
\end{lemma}
\begin{proof}
The set of states $\mathsf{S}^{2}$ is $k$-step determinable and $\mathsf{S}^{1}\subseteq \mathsf{S}^{2}$. 

If $k=0$, we have \[|\mathsf{S}^{1}|=|\mathsf{S}^{2}|=1,\] and \[\mathsf{S}^{1} = \mathsf{S}^{2}.\] Therefore, we have $\mathsf{S}^{1}$ is $0$-step determinable, thus we have the {\em Lemma \ref{lemm:1}} is right when $k=0$.
 
 If for $k=0,\ldots, p$ the {\em Lemma \ref{lemm:1}} is right. When $k=p+1$, we have for $\mathsf{S}^{2}$ there exists a $\mathsf{i}^{i}\in \Delta_M$ such that
 \begin{itemize}
 \item  $|\Ded\left(\mathsf{S}^{2},\mathsf{i}^{i},\varepsilon\right)|=|\mathsf{S}^{2}|$, and 
 \item  for each \[\mathsf{o}^{j}\in \Delta_Q\] such that \[|\Ded(\mathsf{S}^{2},\mathsf{i}^{i},\mathsf{o}^{j})|\neq 0,\] the $\Ded(\mathsf{S}^{2},\mathsf{i}^{i},\mathsf{o}^{j})$ is $k'$-step determinable, where ${k'}<(p+1)$.
 \end{itemize}
 Then we have for $\mathsf{S}^{1}$ there exists the same $\mathsf{i}^{i}\in \Delta_M$ such that
 \begin{itemize}
 \item  $|\Ded\left(\mathsf{S}^{1},\mathsf{i}^{i},\varepsilon\right)|=|\mathsf{S}^{1}|$, and 
 \item  for each \[\mathsf{o}^{j}\in \Delta_Q\] such that \[|\Ded(\mathsf{S}^{1},\mathsf{i}^{i},\mathsf{o}^{j})|\neq 0,\] the \[\Ded(\mathsf{S}^{1},\mathsf{i}^{i}(t),\mathsf{o}^{j})\subseteq \Ded(\mathsf{S}^{2},\mathsf{i}^{i},\mathsf{o}^{j})\] is  $k'$-step deterministic, where ${k'}<(p+1)$.
 \end{itemize}  Then we have the $\mathsf{S}^{1}$ is $(p+1)$-step deterministic. 
