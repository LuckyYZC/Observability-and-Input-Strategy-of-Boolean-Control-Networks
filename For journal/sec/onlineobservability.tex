...
Based on this procedure, we propose the online observability, viz., a \BCN\ is observable if the procedure to determine $\mathsf{s}(0)$ would terminates for every \State$(0)\in \Delta_N$.  \tl{do you mean that the procedure terminates?} \gs{Yes} 
...
As the next step, we introduce a predicate $\Ks(\mathsf{S}(t))$ for $\mathsf{S}(t)$ to depict we can determine the $\mathsf{s}(t)$ in $\Ks(\mathsf{S}(t)$ time steps for every $\mathsf{s}(t)\in \mathsf{S}(t)$. 
