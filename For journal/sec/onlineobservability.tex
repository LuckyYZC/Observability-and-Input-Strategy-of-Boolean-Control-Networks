...
As the next step, we introduce a predicate $\Ks(\mathsf{S}(t))$ for $\mathsf{S}(t)$ to depict that the $\mathsf{s}(t)$ can be determined at time $(t+\Ks(\mathsf{S}(t))$ for every $\mathsf{s}(t)\in \mathsf{S}(t)$. 
...
We propose $\Ri(\mathsf{S}(t))$ to depict that we can choose any $\mathsf{i}(t)$ from $\Ri(\mathsf{S}(t))$ to determine $\mathsf{s}(t)$ at time $t$.
...
We also propose a lemma for $\Ri(\mathsf{S})$. In {\em Section \ref{sec:deter}}, we will design the determinnation algorithm for the online observability based on {\em Lemma \ref{lemm:1}, \ref{lemm:2}}.
\begin{lemma}
