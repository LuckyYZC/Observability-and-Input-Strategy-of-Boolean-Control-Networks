...
We propose $\Ri(\mathsf{S}(t))$ for $\mathsf{S}(t)$, such that we can choose input from $\Ri(\mathsf{S}(t))$ to determine $\mathsf{s}(t)$.
...
Therefore we have this \BCN\ satisfies the online observability, thus we can determine the initial state of it even if its initial state can not be reset. And this \BCN\ is controllable, thus it is identifiable.  %Thus with the online observability we can research the \BCNs\ better.
\label{exa:10}
\end{example}  
...
and the identification problem requires this abiliity. Thus, we can research the identification problem better by the online observability. 