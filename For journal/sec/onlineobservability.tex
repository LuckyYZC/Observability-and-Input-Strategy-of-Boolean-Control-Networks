...
\item[Step 2] With $\mathsf{S}(t)$ we can derive the set of possible inputs $\{\mathsf{i}^z(t),\ldots,\mathsf{i}^w(t)\}$, such that for every $\mathsf{i}(t)\in \{\mathsf{i}^z(t),\ldots,\mathsf{i}^w(t)\}$, for any two distinct $\mathsf{s}(t)$, $\mathsf{s}'(t) \in \mathsf{S}(t)$, $f(\mathsf{s}(t), \mathsf{i}(t))\neq f(\mathsf{s}'(t),\mathsf{i}(t))$. And then we choose $\mathsf{i}(t)$ from $\{\mathsf{i}^z(t),\ldots,\mathsf{i}^w(t)\}$.
...
Clearly, $|\mathsf{S}(t+1)|\le|\mathsf{S}(t)|$, and we can determine $\mathsf{s}(t)$  when $|\mathsf{S}(t)|$ becomes $1$. Because for every $\mathsf{s}(t)\in $ $\mathsf{S}(t)$ there is exact one corresponding $\mathsf{s}(t-1)\in $ $\mathsf{S}(t-1)$, we can determine $\mathsf{s}(t-1)$, $\mathsf{s}(t-2)$, \ldots, $\mathsf{s}(0)$.
...
For a non-empty $\mathsf{S}$
 ...
 \item  if $|\mathsf{S}|>1$ and there is an input $\mathsf{i}^{x} \in \Delta_L$ such that % $\Ks(\mathsf{S})=\min(\{\max\{\Ks(\Ded(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j}))| \Ded(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j})\ne \emptyset, \mathsf{o}^{j}\in\Delta_Q\}||\Ded(\mathsf{S},\mathsf{i}^{x},\varepsilon)|=|\mathsf{S}|, \mathsf{i}^{x} \in \Delta_M\})+1$ 
 \begin{itemize}
