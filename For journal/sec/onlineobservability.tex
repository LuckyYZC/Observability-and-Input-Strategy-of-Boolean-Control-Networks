...
\end{split}
\end{equation*}
where $\varepsilon$ presents empty input. Then the $\Ded( \mathsf{S},  \mathsf{i},  \mathsf{o})$ can be defined as follows.
...
%\end{definition}$\mathsf{S}\in 2^{\mathcal{S}_\BB}$, $\mathsf{i} \in (\mathcal{I}\cup\varepsilon)$, $\mathsf{o} \in (\mathcal{O}\cup\varepsilon)$, and
 ...
 And then, the set $\mathsf{S}(t)$ of possible valuations of $\mathsf{s}(t)$ of a \BCN\ $\BB$ is recursively defined as follows. 
 \begin{definition}[$\mathsf{S}(t)$] Given $\mathsf{i}(0)\ldots\mathsf{i}(t_k)$ and $\mathsf{o}(0)\ldots\mathsf{o}(t_k+1)$, %for every $t\le k$, 
	\[\mathsf{S}(t)=\left\{
\begin{array}{rcl}
...
\end{definition}
\begin{example}
In the \BCN\ in {\em Example \ref{exa:2}}, if $\mathsf{i}(0)\ldots\mathsf{i}(t_k)=\mathsf{i}^2 \mathsf{i}^3$ and $\mathsf{o}(0)\ldots\mathsf{o}(t_k+1)=\mathsf{o}^0 \mathsf{o}^0 \mathsf{o}^1$ then 
\begin{equation*}
\begin{split}
&\mathsf{S}(0)=\Ded\left(\mathcal{S}_\BB,\varepsilon,\mathsf{o}^0\right)=\{\mathsf{s}^0,\mathsf{s}^1,\mathsf{s}^2\}\\
&\mathsf{S}(1)=\Ded\left(\mathsf{S}(0),\mathsf{i}^2,\mathsf{o}^0\right)=\{\mathsf{s}^1,\mathsf{s}^2\}\\
&\mathsf{S}(2)=\Ded\left(\mathsf{S}(1),\mathsf{i}^3,\mathsf{o}^1\right)=\{\mathsf{s}^6\}
 \end{split}
\end{equation*}
...
As the next step, we  recursively define a function $\Ks(\mathsf{S}(t))$ to depict the number of steps we need to determine $\mathsf{s}(t)$ for every $\mathsf{s}(t)\in \mathsf{S}(t)$, that is the $\mathsf{s}(t)$ can be determined at time $(t+\Ks(\mathsf{S}(t))$.
...
In the \BCN\ in {\em Example \ref{exa:2}}, if \[\mathsf{S}(0)=\{\mathsf{s}^0,\mathsf{s}^1,\mathsf{s}^2\},\] then $|\mathsf{S}(0)|=3$, and there exists an input $\mathsf{i}^3$ such that 
 \begin{itemize}
 \item  $|\Ded\left(\mathsf{S}(0),\mathsf{i}^3,\varepsilon\right)|=|\{\mathsf{s}^0,\mathsf{s}^{13},\mathsf{s}^6\}|=|\mathsf{S}(0)|$,
 \item   and for each $\mathsf{o}\in \mathcal{O}$
  \begin{itemize}
  \item   $\Ks(\Ded\left(\mathsf{S}(0),\mathsf{i}^3,\mathsf{o}^0\right))=\Ks(\{\mathsf{s}^0\})=0$;
 \item  $\Ks(\Ded\left(\mathsf{S}(0),\mathsf{i}^3,\mathsf{o}^3\right))=\Ks(\{\mathsf{s}^{13}\})=0$;
  \item  $\Ks(\Ded\left(\mathsf{S}(0),\mathsf{i}^3,\mathsf{o}^1\right))=\Ks(\{\mathsf{s}^6\})=0$.
 \end{itemize}
 \end{itemize}
Thus $\Ks(\mathsf{S}(0))=1$, that is $\mathsf{s}(0)$ can be determined at time $1$ for every $\mathsf{s}(0)\in \mathsf{S}(0)$. 
\end{example}  

And we propose a lemma for $\Ks(\mathsf{S})$.
...
\begin{example}
In the \BCN\ in {\em Example \ref{exa:2}}.  
 \begin{itemize}
 \item $\Ks(\Ded\left(\mathcal{S}_\BB,\varepsilon, \mathsf{o}^0\right))=1\ne \infty$;
 \item $\Ks(\Ded\left(\mathcal{S}_\BB,\varepsilon, \mathsf{o}^1\right))=1\ne \infty$;
 \item $\Ks(\Ded\left(\mathcal{S}_\BB,\varepsilon, \mathsf{o}^2\right))=2\ne \infty$;
 \item $\Ks(\Ded\left(\mathcal{S}_\BB,\varepsilon, \mathsf{o}^3\right))=2\ne \infty$.
 \end{itemize}
...
&\forall \mathsf{o} \in \mathcal{O}\cdot \Ded(\mathsf{S},\mathsf{i},\mathsf{o})\ne \emptyset \rightarrow \Ks(\Ded(\mathsf{S},\mathsf{i},\mathsf{o}))\ne \infty\}
\end{split}
\end{equation}
%\end{definition}
