...
\end{itemize} 
 ...
 The cardinal number of possible states set does not change or decrease in the determining process i.e., $|\mathsf{S}(t+1)|\le|\mathsf{S}(t)|$ (will be shown in {\em Example \ref{exa:8}}). If the cardinal number of possible states set $|\mathsf{S}(t)|=1$, then we can determine the $\mathsf{s}(t)$, and then determine $\mathsf{s}(0)$. 
...
Therefore, we have the definition of the online observability that a \BCN\ is online observable if every set possible states $\mathsf{S}^{i}(0)$ we derived at time step $0$ there exists a $k^i\in \mathbb{N}$ such that the $\mathsf{S}^{i}(0)$ is $k^i$-step determinable.
...
\item  If for $k=0,\ldots, p$ the $k$-step determinability is well defined, where $p\ge 0$. When $k=p+1$, then we can determine the state $\mathsf{s}^{i}(t)$ at time step $t+(p+1)$ for every $\mathsf{s}^{i}(t)\in \mathsf{S}(t)$. To guarantee this, we have for $\mathsf{S}(t)$ there exists an input $\mathsf{i}^{i}(t) \in \Delta_M$ such that
\begin{itemize}
...
\begin{lemma}
For the sets of possible states $\mathsf{S}^{x}(t)$ and $\mathsf{S}^{y}(t)$, if there exists a $k^{i}\ge 0$ such that $\mathsf{S}^{y}(t)$ is $k^{i}$-step determinable, and $\mathsf{S}^{x}(t)\subseteq \mathsf{S}^{y}(t)$, then there exists a $k^{j}\ge 0$ that $\mathsf{S}^{x}(t)$ is $k^{j}$-step determinable.
\label{lemm:3}
\end{lemma}
\begin{proof}For the set of states $\mathsf{S}^{y}(t)$, there exists a $k^{i}\ge 0$ such that $\mathsf{S}^{y}(t)$ is $k^{i}$-step determinable, and $\mathsf{S}^{x}(t)\subseteq \mathsf{S}^{y}(t)$. From the {\em Lemma \ref{lemm:4}}, we have $\mathsf{S}^{x}(t)$ is $k_i$-step deterministic too, thus for the $\mathsf{S}^{x}(t)$ there exists a $k^{j}=k^{i}\in \mathbb{N}$ such that $\mathsf{S}^{x}(t)$ is $k^{j}$-step determinable..
\end{proof}

With the {\em Lemma \ref{lemm:3}}, we have its equivalent proposition.
For the sets of states $\mathsf{S}^{x}(t)$ and $\mathsf{S}^{y}(t)$, if there does not exist any $k^{i}\ge 0$ such that $\mathsf{S}^{x}(t)$ is $k^{i}$-step determinable, and $\mathsf{S}^{x}(t)\subseteq \mathsf{S}^{y}(t)$, then there does not exist a $k^{j}\in \mathbb{N}$ that make $\mathsf{S}^{y}(t)$ $k^{j}$-step determinable.
