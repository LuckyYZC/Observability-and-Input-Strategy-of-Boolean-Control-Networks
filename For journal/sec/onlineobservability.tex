...
\[\mathsf{S}(1)=\{\mathsf{s}(1)|\mathsf{s}(0)\in \mathsf{S}(0),\ \mathsf{s}(1)=L\ltimes{\mathsf{i}(0)}\ltimes{\mathsf{s}(0)},\ H \ltimes\mathsf{s}(1)=\mathsf{o}(1)\}.\]
...
\begin{itemize}
  ...
  \item  and for every $\mathsf{s}^{x}(1)$ $\in \mathsf{S}^{i}(1)$ there exists only one corresponding $\mathsf{s}^{x}(0)$ $\in \mathsf{S}^{i}(0)$ such that $\mathsf{s}^{x}(1)=L\ltimes{\mathsf{i}(0)}\ltimes{\mathsf{s}^{x}(0)}$,
\end{itemize} 
...
\item Secondly, with the set of possible states $\mathsf{S}(t)$ we can derive the set of possible inputs $\mathsf{I}(t)$, such that for every $\mathsf{i}^{i}(t)\in \mathsf{I}(t)$ we have  for any distinct $\mathsf{s}^{x}(t)$, $\mathsf{s}^{y}(t) \in \mathsf{S}(t)$, they will not become the same state after being affected by the input $\mathsf{i}^{i}(t)$ i.e., $L\ltimes\mathsf{s}^{x}(t)\ltimes \mathsf{i}^{i}(t)\neq L\ltimes\mathsf{s}^{y}(t) \ltimes\mathsf{i}^{i}(t)$. This principle will be shown in the third subsection.  And then, we choose an input $\mathsf{i}(t)$ from $\mathsf{I}(t)$.
\item Thirdly, based on the relation between input and state of \BCN\ $\mathsf{s}(t+1)= L\ltimes{\mathsf{i}(t)}\ltimes{\mathsf{s}(t)}$, the set of possible states $\mathsf{S}(t+1)$ of next time step ($t+1$) is preliminarily derived by the input $\mathsf{i}(t)$ we chose. 
\end{itemize} 
...
\item If for $k^{i}=0,\ldots, p$ the propostion is right. When $k^{i}=p+1$, we have for $\mathsf{S}(t)$ there exists a $\mathsf{i}^{i}(t)\in \Delta_M$ such that
 ...
 \end{itemize}
 Then we have for every state \State$^{x}(t)$$\in \mathsf{S}(t)$, there exists an input sequence $\mathsf{I}\in(\Delta_M)^k$ such that for all states $\mathsf{s}^{x}(t)\neq \mathsf{s}^{y}(t)\in \mathsf{S}(t)$ implies $(HL)^k_{\mathsf{s}^{y}(t)}(\mathsf{I})\neq (HL)^k_{{\mathsf{s}^{x}(t)}}(\mathsf{I})$. Where the input sequence $\mathsf{I}=\mathsf{i}^{i}(t)\mathsf{I}'$. And the input sequence $\mathsf{I}'$ satisfies the following properties, %for every state \State$^{x}(t)$$\in \mathsf{S}(t)$
  for all states \[\mathsf{s}^{x}(t+1)\neq \mathsf{s}^{z}(t+1)\in \Ded(\mathsf{S}(t),\mathsf{i}^{i}(t),H\ltimes\mathsf{s}^{x}(t+1))\] implies \[(HL)^p_{\mathsf{s}^{z}(t+1)}(\mathsf{I}')\neq (HL)^p_{{\mathsf{s}^{x}(t+1)}}(\mathsf{I}').\] Where $\mathsf{s}^{x}(t+1)=L\ltimes\mathsf{s}^{x}(t)\ltimes\mathsf{i}^{i}(t)$. Then we have the propostion is right when $k^{i}=p+1$. 
...
Secondly, we have that if a \BCN\ satisfies the third observabbility, then there exists an input sequence $\mathsf{I}\in(\Delta_M)^k$ for some $k\in \mathbb{Z}_+$, such that for any distinct states $\mathsf{s}^{i}(0)$, $\mathsf{s}^{j}(0) \in \Delta_N$, $H\ltimes\mathsf{s}^{i}(0)=H\ltimes\mathsf{s}^{j}(0)$ implies $(HL)^k_{\mathsf{s}^{i}(0)}(\mathsf{I})\neq (HL)^k_{\mathsf{s}^{j}(0)}(\mathsf{I})$. 
