...
For a non-empty set $\mathsf{S}\subseteq\Delta_M$
 ...
 \item   for every $\mathsf{o} \in \Delta_N$, $\Ded(\mathsf{S},\mathsf{i},\mathsf{o})\ne \emptyset$ implies $\Ks(\Ded(\mathsf{S},\mathsf{i},\mathsf{o}))< k$;
 \end{itemize} 
 and there is not any $\mathsf{i}' \in \Delta_L$ which satisfies that
  \begin{itemize}
 \item  $|\Ded(\mathsf{S},\mathsf{i}',\varepsilon)|=|\mathsf{S}|$, and 
...
As mentioned in the {\em Example \ref{exa:8}}, $|\mathsf{S}(0)|=3$, and there exists $\delta_{4}^3$ such that 
 ...
 \item   and for each $\mathsf{o}\in \Delta_N$
  \begin{itemize}
...
%\tl{(12)why $\epsilon$ is in the def? for which purpose?}
 A \BCN\ is observable
if for every $\mathsf{o}(0)\in  \Delta_N$, $\Ded(\Delta_M,\varepsilon, \mathsf{o}(0))\ne\emptyset$ implies $\Ks(\Ded\left(\Delta_M,\varepsilon,\mathsf{o}(0)\right))\ne\infty$.
\end{definition}
