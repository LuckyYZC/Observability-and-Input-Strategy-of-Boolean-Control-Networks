...
Instead of finding the input sequence $\mathsf{I}$ before we take the procedure of determining the initial state. In the online observability we use the set of possible states $\mathsf{S}(t)$ (it will be formally defined in the next subsection) we derive at every time step $t$ to adaptively construct the $\mathsf{I}$ to determine the $\mathsf{s}(0)$ of \BCN. 
...
%-------------------------------------
{\color{red} (10)} To describe how to derive $\mathsf{S}(t)$ by $\mathsf{o}(t)$, $\mathsf{i}(t-1)$, $\mathsf{S}(t-1)$ and the updating rules of \BCN, we define the derivation function. %The definition of it is as follows.
...
Then, we propose $\Ri(\mathsf{S}(t))$ to depict how to derive the $\mathsf{i}(t)$ by $\mathsf{S}(t)$ in the process of determining $\mathsf{s}(0)$.
