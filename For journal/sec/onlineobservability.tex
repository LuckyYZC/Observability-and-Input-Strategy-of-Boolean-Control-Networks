...
For a non-empty $\mathsf{S}$
 ...
 \item  $\Ks(\Ded\left(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j}\right))< k$ for each non-empty $\Ded\left(\mathsf{S},\mathsf{i}^{x},\mathsf{o}^{j}\right)$.
% \item and there is a $\Ks(\Ded\left(\mathsf{S},\mathsf{i}^{i},\mathsf{o}^{j}\right))= k-1$,
 ...
 \item  $\Ks(\Ded\left(\mathsf{S},\mathsf{i}^{y},\mathsf{o}^{j}\right))< k-1$ for each non-empty $\Ded\left(\mathsf{S},\mathsf{i}^{y},\mathsf{o}^{j}\right)$.
% \item and there is a $\Ks(\Ded\left(\mathsf{S},\mathsf{i}^{i},\mathsf{o}^{j}\right))= k-1$,
 \end{itemize} 
...
In the \BCN\ mentioned in {\em Example \ref{exa:2}}, if $\mathsf{o}(0)=\delta_4^0$, then $\mathsf{S}(0)=\Ded\left(\Delta_N,\varepsilon,\delta_4^0\right)=\{\delta_{16}^0,\delta_{16}^1,\delta_{16}^2\}$, $|\mathsf{S}(0)|=3$, and for $\mathsf{S}(0)$ there exists $\delta_{4}^3$ such that 
 ...
 \end{itemize}
 \end{itemize}
 Therefore $\Ks(\mathsf{S}(0))=1$, we can  input $\delta_{4}^3$ to determine $\mathsf{s}(0)$ at time step $1$.
\end{example}  

In addition, we propose some lemmas for the $k$-step determinability.

\begin{lemma}
For the non-empty $\mathsf{S}^{1}$ and $\mathsf{S}^{2}$, if $\mathsf{S}^{1}\subseteq\mathsf{S}^{2}$ then $\Ks(\mathsf{S}^{1})\le\Ks(\mathsf{S}^{2})$.%and $\mathsf{S}^{1}\subseteq \mathsf{S}^{2}$, 
  \label{lemm:1}
\end{lemma}
