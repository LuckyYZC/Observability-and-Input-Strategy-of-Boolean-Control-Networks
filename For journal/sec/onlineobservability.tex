...
As $|\mathsf{S}(t+1)|\le|\mathsf{S}(t)|$,we can determine $\mathsf{s}(t)$  when $|\mathsf{S}(t)|$ becomes $1$. And because for every $\mathsf{s}^{i}(t)\in $ $\mathsf{S}(t)$ there is exact one corresponding $\mathsf{s}^{i}(t-1)\in $ $\mathsf{S}(t-1)$, we can determine $\mathsf{s}(t-1)$, $\mathsf{s}(t-2)$, \ldots, and then $\mathsf{s}(0)$.
...
\item  Secondly, we need to describe how to derive every $\mathsf{i}(t)$ by $\mathsf{S}(t)$ such that  we can determine $\mathsf{s}(0)$. %Thus we define the $k$-step determinability for $\mathsf{S}(t)$ which means that we can determine $\mathsf{s}(t)$ by $\mathsf{S}(t)$ in $k$ time steps.
\end{itemize} 
...
%-------------------------------------
{\color{red} (9)} To describe how to derive $\mathsf{S}(t)$ by $\mathsf{o}(t)$, $\mathsf{i}(t-1)$, $\mathsf{S}(t-1)$ and the updating rules of \BCN, we define the derivation function $\Ded\left( \mathsf{S},  \mathsf{i},  \mathsf{o}\right)$. %The definition of it is as follows.
...
Then, we propose $\Ri(\mathsf{S}(t))$ to depict how to derive the $\mathsf{i}(t)$ by $\mathsf{S}(t)$ in the process of determining $\mathsf{s}(0)$. We can choose any $\mathsf{i}(t)\in \Ri(\mathsf{S}(t))$ to determine $\mathsf{s}(0)$
...
{\em Lemma \ref{lemm:2}} can help us design the algorithm to determine the online observability of a \BCN.%, and we will represent the details in Section \ref{sec:deter}.
%==========================================================================================
