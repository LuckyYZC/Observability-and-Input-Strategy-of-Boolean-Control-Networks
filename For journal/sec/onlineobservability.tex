...
%-------------------------------------
\subsection{Definition of online observability}
%-------------------------------------
To formally define the online observability, we start with the derivation function $\Ded( \mathsf{S},  \mathsf{i},  \mathsf{o})$ which formalize how to determine the set $\mathsf{S}(t)$ of possible valuations of $\mathsf{s}(t)$ of a \BCN\ by its $\mathsf{o}(t)$, $\mathsf{i}(t-1)$, $\mathsf{S}(t-1)$ and updating rules.  %The definition of it is as follows.
...
If $\mathsf{o}\neq \varepsilon$
\begin{equation*}
 \Ded( \mathsf{S},  \mathsf{i},  \mathsf{o})=\{  \xi(\mathsf{i}, s ) \mid  \mathsf{s} \in \mathsf{S},  \rho(\xi(\mathsf{i}, s ))=\mathsf{o}\}
\end{equation*}
...
\end{definition}
 ...
 Then we have the set $\mathsf{S}(t)$ of possible valuations of $\mathsf{s}(t)$ of a \BCN\ $\BB$ is defined as follows. 
...
For a non-empty set $\mathsf{S}\subseteq\Delta_M$
 \begin{itemize}
 ...
 \end{itemize} 
 and there is not any $\mathsf{i}' \in \Delta_L$ satisfies that
  \begin{itemize}
 \item  $|\Ded(\mathsf{S},\mathsf{i}',\varepsilon)|=|\mathsf{S}|$, and 
...
%\tl{(12)why $\epsilon$ is in the def? for which purpose?}
 A \BCN\ is observable,
if for every $\mathsf{o}^{j}(0)\in  \Delta_N$, $\Ded(\Delta_M,\varepsilon, \mathsf{o}^{j}(0))\ne\emptyset$ implies $\Ks(\Ded\left(\Delta_M,\varepsilon,\mathsf{o}^{j}(0)\right))\ne\infty$.
\end{definition}
...
\subsection{Determining the iintial state}

 If a \BCN\ $\BB$ with the online observability, then its initial state $\mathsf{s}(0)$ can be determined by followng procedures without resetting $\mathsf{s}(0)$.
...
\begin{description}
	...
	\item[Step 2] Input to the \BCN\ $\BB$ with an $\mathsf{i}$ which belongs to $\Ri(\mathsf{S})$, and run $\BB$ to generate the new output $\mathsf{o}(t)$. 
...
%From the Step 2, we have for every $ k=1,\ldots, t$, for every $\mathsf{s}'(k)\in \mathsf{S}(k)$, there is only one corresponding $\mathsf{s}'(k-1)\in \mathsf{S}(k-1)$ such that the $\mathsf{s}'(k)$ is determined by $\mathsf{s}'(k-1)$ and $\mathsf{i}(k-1)$. Thus %once $|\mathsf{S}|=1$, we can determine $\mathsf{s}(t)$, and then determine $\mathsf{s}(t-1),\dots,\mathsf{s}(0)$.
 ...
 In Step 4, when $|\mathsf{S}|=1$, for the input sequence $\mathsf{I}[t]$, there is only one initial state $\mathsf{s}'(0)$ whose corresponding output sequence  $\mathsf{O}'[t]$ equals to the output sequence $\mathsf{O}[t]$ of $\BB$, thus, we can determine the initial state by the input sequence $\mathsf{I}[t]$ and output sequence $\mathsf{O}[t]$.
...
We call this observability online observability, because we choose every $\mathsf{i}(t)\in\Ri(\mathsf{S}(t))$ based on the information of the inputs and outputs of \BCN\ we have collected so far at every time $t$. 
...
In contrast, in the {\bf type-III} observability, we determine the initial state $\mathsf{s}(0)$ of a \BCN\ by its recorded $\mathsf{O}[t]=\mathsf{o}(0)\ldots\mathsf{o}(t)$ after we input $\mathsf{I}[t]=\mathsf{i}(0)\ldots\mathsf{i}(t-1)$, and  we do not interfere with \BCN\ except for the logging of its inputs and outputs, thus we call it {\em offline observability}.
...
In conclusion, the online observability captures the ability to determine the initial state $\mathsf{s}(0)$ of \BCN\ whose initial state $\mathsf{s}(0)$ can not be reset, %Because at every time $t$, we observe $\mathsf{o}(t)$ of \BCN\ and choose $\mathsf{i}(t)$ based on the information we have collected so far. 
and the identification problem requires this abiliity. Thus, we can research the identification problem by the online observability instead of the {\bf Type-III} observability. 