...
\begin{problem}
\label{pro:1}
For a given \BCN\ $\BB$, whether there is an $\mathsf{i}(0)\mathsf{i}(1)\ldots\mathsf{i}(k-1)$, such that the initial state $\mathsf{s}(0)$ can be determine by $\mathsf{i}(0)\mathsf{i}(1)\ldots\mathsf{i}(k-1)$?
\end{problem}
...
%In other words, only third and fourth notions of observability can help us solve the {\em Problem~\ref{pro:1}}.
