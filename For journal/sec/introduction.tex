...
Each of these types of observability is used to solve different problems of \BCNs. The {\bf type-I}  observability is proposed to determine the initial state $\mathsf{s}(0)$ of \BCN\ with the assumption that its (unknown) initial state $\mathsf{s}(0)$ of a \BCN\ can be reset, or alternatively having a number of copies of the original \BCN, all in the same initial state. The {\bf type-II}  observability is proposed to be the more standard observability to determine the initial state $\mathsf{s}(0)$ of \BCN\ with the assumption that its  (unknown) initial state $\mathsf{s}(0)$ can be reset, i.e. if its $\mathsf{s}(0)$ can be determined iff it satisfies the  {\bf type-II}  observability. The {\bf type-III} observability is used to research the {\em identification problem} of \BCN, and the {\bf type-IV} observability is used to research the reconstructibility of \BCN. %And we found the {\bf type-III} observability not good enough for the the identification problem.
...
There further states that a \BCN\ is identifiable iff it satisfies controllability and its initial state can be determined with the condition that its (unknown) initial state $\mathsf{s}(0)$ can not be reset. %, we have that we need to determine the initial state of a \BCN\ to identify its updating rules.
...
There is an input sequence $\mathsf{I}[k]=\mathsf{i}(0)\ldots\mathsf{i}(k-1)$ for some $k>0$ such that for all ${2^m}$ different initial states $\mathsf{s}^1(0),\ldots, \mathsf{s}^{2^m}(0)$ of $\BB$, their corresponding output sequence $\mathsf{O}^1[k]=\mathsf{o}^1(0)\ldots\mathsf{o}^1(k),\ldots, \mathsf{O}^{2^m}[k]=\mathsf{o}^{2^m}(0)\ldots\mathsf{o}^{2^m}(k)$ are different. Thus, we have followng procedure to determe its initial state $\mathsf{s}(0)$.
...
Let $\mathsf{S}(0)$ denote the set of possible valuations of $\mathsf{s}(0)$. At every time $t$, we obtain it by the information about the input and output of \BCN\ we have collected so far. When $t=0$, for every $\mathsf{s}'(0)\in\mathsf{S}(0)$, $\mathsf{s}'(0)$ produces $\mathsf{o}(0)$. When $t>0$, for every $\mathsf{s}'(0)\in\mathsf{S}(0)$, the output sequence $\mathsf{O}[t]=\mathsf{o}(0)\ldots\mathsf{o}(t)$ can be produced by the input sequence $\mathsf{I}[t]=\mathsf{i}(0)\ldots\mathsf{i}(t-1)$ and $\mathsf{s}'(0)$. Let $\mathsf{S}(t)$ denote the set of possible valuations of $\mathsf{s}(t)$ which is produced by $\mathsf{S}(0)$ and $\mathsf{I}$.
The new algorithm we proposed is described as follows. 
