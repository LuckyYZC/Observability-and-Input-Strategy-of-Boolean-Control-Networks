...
As shown in Fig.~\ref{fig:10}, a \BCN\ has three distinguished finite sets of nodes $\{\mathsf{i}_1,\ldots\mathsf{i}_\ell\}$, $\{\mathsf{s}_1,\ldots\mathsf{s}_m\}$ and $\{\mathsf{o}_1,\ldots\mathsf{o}_m\}$ for some natural numbers $\ell$, $m$ and $n$, which are called the {\em input-nodes}, {\em state-nodes}  and {\em output-nodes}, respectively. At any time each node takes a Boolean value. The value $\mathsf{o}_i(t)$ of an output-node $\mathsf{o}_i$ at a time point $t$ for $t\ge0$ is determined by the values of the state-nodes at time $t$, that is , there exist Boolean functions $f_i$ for $i=1,\ldots,n$ such that $\mathsf{o}_i(t)=f_i(\mathsf{s}_1(t),\ldots,\mathsf{s}_m(t))$; and the value $\mathsf{s}_j(t+1)$ of a state-node at time point $t+1$ is determined by the values of the input-nodes and state-nodes at time point $t$ by given functions $\mathsf{s}_j(t+1)=g_j(\mathsf{i}_1,\ldots,\mathsf{i}_\ell,\mathsf{s}_1(t),\ldots,\mathsf{s}_m(t))$ for $j=1,\ldots,m$. These functions are called the {\em updating rules} of the \BCN. 
...
Loosely speaking, different types of observability proposed for different applications. For instance, the {\bf type-IV} observability is proposed to research the reconstructibility of \BCN, and the {\bf type-III} observability is proposed to research the {\em identification problem} of \BCN. And we found the {\bf type-III} observability not good enough for the the identification problem.
...
%{\em Observability}:A \BCN\ $\BB$ is observable if there is an input sequence $\mathsf{i}(0)\ldots\mathsf{i}(k-1)$ that distinguishes every pair of different initial states. This means that in $\BB$, there exists an input sequence  $\mathsf{i}(0)\ldots\mathsf{i}(k-1)$ for some $k>0$ such that for any two different initial states $\mathsf{s}(0)$ and $\mathsf{s}'(0)$, the output sequence $\mathsf{o}(0)\ldots \mathsf{o}(k)$  corresponding to  $\mathsf{s}(0)$  is different from the output sequence  $\mathsf{o}'(0)\ldots\mathsf{o}'(k)$ corresponding to $\mathsf{s}'(0)$.
 ...
 From \cite{Cheng2011Identification}, we have we need to determine the initial state of \BCNs\ to identify their updating rules. And if a \BCN\ $\BB$ satisfies the {\bf type-III} observability, then we can determine its initial state $\mathsf{s}(0)$ by the following algorithm.
...
Assume a \BCN\ $\BB$  with the {\bf type-III} observability. Then, there is an input sequence $\mathsf{i}(0)\ldots\mathsf{i}(k-1)$ that distinguishes every pair of different initial states $\mathsf{s}'(0), \mathsf{s}''(0)$ of $\BB$. Thus, we have foloiwng procedure to determe its initial state $\mathsf{s}(0)$.
...
Thus, in \cite{Cheng2011Identification},  the author claims that the identification problem of a \BCN\ requres the {\bf type-III} observability.
