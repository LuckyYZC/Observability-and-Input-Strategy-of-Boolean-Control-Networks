...
the node's state     can change in different  time step.  For a node $n$, we use $n(t)$ to denote the state of $n$ at time step $t$.

The value of each node $n_i(t+1)$ is decided by logical function of  set of  values  $n_j(t)$  where  there is a directed edge from $n_j$ to $n_i$. We call nodes as $n_j$ are the neighboring nodes of $n_i$ \ly{In \BNs,   $n_i(t+1)$ is obtained through logical operations on the  neighboring nodes' value at time $t$.} 
