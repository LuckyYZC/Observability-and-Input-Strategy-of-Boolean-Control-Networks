...
These two functions are called the {\em updating rules} of $\BB$. A {\em timed run}  (or {\em execution}) up to any time point $t$ of $\BB$ is a sequence $\mathsf{I}[t] = \mathsf{i}(0)\ldots \mathsf{i}(t)$ of inputs, a sequence $\mathsf{S}[t] = \mathsf{s}(0)\ldots \mathsf{s}(t)$ of states, and a sequence $\mathsf{O}[t] = \mathsf{o}(0)\ldots \mathsf{o}(t)$  of outputs. The executions $\mathsf{S}[t]$ and $\mathsf{O}[t]$ from an initial state $\mathsf{s}(0)$ is driven by the input sequence $\mathsf{i}(0)\ldots \mathsf{i}(t-1)$.
