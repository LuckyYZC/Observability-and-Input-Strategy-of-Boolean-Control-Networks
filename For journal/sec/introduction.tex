...
The $n_i(t+1)$ is decided by a logical function of  set of  values  $\{n_j(t),\ldots,n_p(t)\}$ if  there are directed edges from $n_j,\ldots,n_p$ to $n_i$.  % We call nodes as $n_j,\ldots,n_p$ are the neighboring nodes of $n_i$ %\ly{In \BNs,   $n_i(t+1)$ is obtained through logical operations on the  neighboring nodes' value at time $t$.} 
 The logical operators used in  logical function including AND, OR, NO, XOR. %\ly{, and so on, "delete"}. 
...
\end{comment}
Although, four existing observability can help us study some information about $\mathsf{s}(0)$ we need. But when the determining procedure can do at most once, only third and fourth observability  can  determine the $\mathsf{s}(0)$ of \BCNs. %{\color{blue}{Not clear}} And conditions of third and fourth in above observability are strong.
...
For a given \BCN\  $\BB$. With its output sequence $\mathsf{o}(0)\mathsf{o}(1)\ldots\mathsf{o}(t)$ and input sequence $\mathsf{i}(0)\mathsf{i}(1)\ldots\mathsf{i}(t-1)$, we can infer the possible valuation of its $\mathsf{s}(t)$ by its updating rules. And  we use $\mathsf{S}(t)$ to denote the set of possible valuations of $\mathsf{s}(t)$. %\tl{where do you define potential valuations?}
\begin{itemize}
