...
Inspired by this, we propose the online observability in this paper. It means that we adaptively contruct the input sequence $\mathfrak{i}(0)$$\mathfrak{i}(1)\ldots$$\mathfrak{i}(k-1)$ to determine the initial state $\mathfrak{s}(0)$ by the $\mathcal{S}(t)$ we derived at every time step. %Because we find input sequence by the \Ustate$(t)$ we derived at every time step, we call this type of observability online observability. 
