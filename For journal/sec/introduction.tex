...
\begin{itemize}
	...
	\item Thirdly, we have $|$$\mathsf{S}(t)$$|\le|$$\mathsf{S}(t-1)$$|$. When $|$$\mathsf{S}(t)$$|=1$, we can determine $\mathsf{s}(t)$. Because for every $\mathsf{s}^{x}(t)\in $ $\mathsf{S}(t)$ there is exact one corresponding $\mathsf{s}^{x}(t-1)\in $ $\mathsf{S}(t-1)$ we can determine $\mathsf{s}(t-1)$, $\mathsf{s}(t-2)$, \ldots, $\mathsf{s}(0)$.
\end{itemize}
...
{\color{red} (13)} Inspired by this, we propose the online observability which states that a \BCN\ $\BB$ is observable if the previously mentioned procedure can be completed {\bf definitely}. Its formal definitions will be presented in Section~\ref{sec:online}. It address the necessary and sufficient condition of determining $\mathsf{s}(0)$ by taking the determining procedure once. Because in the online observability, we adaptively construct every $\mathsf{i}(t)$ by the $\mathsf{S}(t)$ we derived, with different $\mathsf{S}(t)$, the $\mathsf{i}(t)$ can be different. While in the third observability the $\mathsf{i}(t)$ have to be pre-specified.
