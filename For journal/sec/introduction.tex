...
Let $\mathsf{S}(0)$ denote the set of possible valuations of $\mathsf{s}(0)$. When $t=0$, assume $\mathsf{o}(0)$ is the output of the \BCN, then for every $\mathsf{s}'(0)\in\mathsf{S}(0)$, $\mathsf{s}'(0)$ produces $\mathsf{o}(0)$. When $t>0$, assume $\mathsf{I}=\mathsf{i}(0),\mathsf{i}(1),\ldots,\mathsf{i}(t-1)$ and $\mathsf{O}=\mathsf{o}(0),\mathsf{o}(1),\ldots,\mathsf{o}(t)$ present the input sequence and the output sequence of \BCN, respectively, then for every $\mathsf{s}'(0)\in\mathsf{S}(0)$, $\mathsf{s}'(0)$ and $\mathsf{I}$  produce  $\mathsf{O}$.
...
\begin{description}
	...
	\item[Step 4] Set $\mathsf{S}=\mathsf{S}\cap\mathsf{S}(0)$ by the intersection of $\mathsf{S}(0)$ and $\mathsf{S}$. If the cardinal number of $\mathsf{S}$ equal to $0$, then return the $\mathsf{s}'(0)$ belonging to $\mathsf{S}$ as the initial state of $\BB$. Otherwise, and go to Step 2.
\end{description}
...
\begin{description}
	\item[Step 1]  Set variable $\mathsf{S}$ be set to $\mathsf{S}(0)$ ($t=0$).
	\item[Step 2] Input to the \BCN\ $\BB$ (whose initial state $\mathsf{s}(0)$ is to be determined) with the an $\mathsf{i}(t)$ which would not make every two distinct $\mathsf{s}'(t)$ , $\mathsf{s}''(t)$$\in$ $\mathsf{S}$ become the same state after affected by $\mathsf{i}(t)$ i.e. $\mathsf{s}'(t+1)\ne\mathsf{s}''(t+1)$. 
	\item[Step 3] With the input sequence $\mathsf{i}'(0),\mathsf{i}'(1),\ldots, \mathsf{i}'(t)$ and output sequence $\mathsf{o}(0),\mathsf{o}(1),\ldots,\mathsf{o}(t+1)$, we can get the new $\mathsf{S}(0)$ ($(t+1)>0$).
	\item[Step 4] Set $\mathsf{S}=\mathsf{S}\cap\mathsf{S}(0)$ by the intersection of $\mathsf{S}(0)$ and $\mathsf{S}$. If the cardinal number of $\mathsf{S}$ equal to $0$, then return the $\mathsf{s}'(0)$ belonging to $\mathsf{S}$ as the initial state of $\BB$. Otherwise, and go to Step 2.
\end{description}
