...
\end{enumerate}
 ...
 From the four notions of observability, we can see that the first and second observability determine $\mathsf{s}(0)$ by carrying on multiple parallel experiments, and the third and fourth observability is by carrying on one experiment. And the second observability is the necessary and sufficient condition of determine $\mathsf{s}(0)$ by carrying on multiple parallel experiments. In this paper, we want to solve the 
following problem:
\begin{problem}
\label{pro:1}
For a given \BCN\ $\BB$, whether its initial state $\mathsf{s}(0)$ can be determined by carrying on one experiment?
\end{problem}
...
%However, 
We find that to determine $\mathsf{s}(0)$ by carrying on one experiment we only need to complete the following procedure.
