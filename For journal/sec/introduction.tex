...
This about if it is possible and how for a given initial state to find a sequence of inputs such that the behavior of a \BCN\ $\BB$ corresponding to the sequence of inputs will reach a desired state~\cite{Akutsu2007Control}.
...
{\em The observability problem}: This is about it is possible and how to determine the intial state of a \BCN\ $\BB$ by providing sequences of inputs and observing the corresponding output sequences ~\cite{cheng2009controllability}. There are four types of observability have been investigated in the literature, which are described below.
...
For the $\mathsf{S}(t)$ of a \BCN\ $\BB$, we find that
 \begin{itemize}
	...
	\item We can determine the $\mathsf{s}(t)$ of $\BB$ at time point $t+k$ for some $k>0$ for every $\mathsf{s}(t)\in \mathsf{S}(t)$, iff there is an input $\mathsf{i}(t)$ 
	\begin{itemize}
...
Then, we propose a new type of observability named {\em online observability} which states that a \BCN\ is online observable if for every possible output $\mathsf{o}(0)$ we may observe, the $\mathsf{S}(0)$ determined by $\mathsf{o}(0)$ satisfies that we can determine the $\mathsf{s}(0)$ of $\BB$ at time point $t+k$ for some $k\ge0$ for every $\mathsf{s}(0)\in \mathsf{S}(0)$. Its formal definition will be presented in Section~\ref{sec:online}. If a \BCN\ $\BB$ with the online observability, we have foloiwng procedure to determe its initial state $\mathsf{s}(0)$.
