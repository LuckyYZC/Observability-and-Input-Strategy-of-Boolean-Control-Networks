...
%\STATE  $N_i$ (Node)\
%\STATE  $i_p$ (Input)\
\STATE node $NodesArray[NM]$ %=$\Delta_N$\
\STATE  node $NewArray[NM]$ %=$\Delta_N$\
\STATE  node $TemArray[M]$ %=$\Delta_N$\
\STATE $NodesArray[0]$ =$\Delta_N$\
%\STATE  $Sis$ (The suitable inputs set of $N_i$)\
%\STATE {\sf buildnode}(k)
%\STATE $k= k+1$
\WHILE {$Ob==$ false}
\STATE  $j=0$ \
\FOR{$i=0$; $i<arraysize(NodesArray)$; $i++$}
\STATE  $TemArray= $Child nodes of $NodesArray[i]$ 
...
\STATE $NodesArray[i]=$ Child node of $NewArray[j]$
\STATE $i++$
\ENDIF
\ENDFOR
\ENDIF
