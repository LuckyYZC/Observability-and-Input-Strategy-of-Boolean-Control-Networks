...
\end{itemize}	
Each node at any time takes a Boolean value in $\mathbb{B}$, and the $\BB$'s {\bf updating rules}  $\sigma$ and $\rho$ are Boolean valued functions which define relations among the values of the nodes such that for any time $t\in  \mathbb{T}$
\begin{equation}
\begin{split}
