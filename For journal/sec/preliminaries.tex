...
\item The Boolean valued functions $\sigma: \mathbb{B}^\ell \mathbb{B}^m \mapsto \mathbb{B}^m$ and $\rho: \mathbb{B}^m\mapsto \mathbb{B}^n$ are functions from the pairs of $\ell$-dimension and $m$-dimension vectors  of Boolean values to  the $m$-dimension vectors Boolean values and from the  $m$-dimension vectors to the  $n$-dimension vectors  of Boolean values, respectively. 
\item {\em Timed behavior}: each node $v\in I\cup S\cup O$ represents a Boolean variable, and we use 
 $i= (i_1,\ldots, i_\ell)$, $s= (s_1,\ldots, s_m)$ and $o= (o_1,\ldots, o_n)$ to denote the three Boolean variables vectors corresponding  to the input nodes, state nodes and output nodes. At any time $t\in \mathbb{T}$ during the execution 
