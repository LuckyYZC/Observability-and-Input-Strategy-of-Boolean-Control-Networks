...
\begin{itemize}
  \item The {\bf Type-I} observability is that, a \BCN\ is observable if for every initial state $\mathsf{s}\in \mathcal{S}$, there exists an input sequence $\mathsf{I}[t]\in\mathcal{I}^{[t]}$ for some $t\in\mathbb{T}$, such that for all states $\mathsf{s}'$, $H^{[0,t]}(\mathsf{s}'(0),\mathsf{I}[t])\neq H^{[0,t]}(\mathsf{s}(0), \mathsf{I}[t])$ if $\mathsf{s}\ne\mathsf{s}'$.
...
\begin{example}
For instance, in the \BCN\ mentioned in {\em Example \ref{exa:2}}, for any $t\in \mathbb{T}$, there is not an $\mathsf{I}[t]\in\mathcal{I}^{[t]}$ which can distinguish every two distinct initial states $\mathsf{s}$, $\mathsf{s}'$:
\begin{itemize}
...
\begin{itemize}\label{exa:3}
  \item we can distinguish the sets of states 
  \begin{itemize}
   \item $\{\delta_{16}^0,\delta_{16}^{1},\delta_{16}^{2}\}$,
   \item $\{\delta_{16}^3,\delta_{16}^{4},\delta_{16}^{5},\delta_{16}^{6}\}$,
   \item $\{\delta_{16}^7,\delta_{16}^{8},\delta_{16}^{9},\delta_{16}^{10}\}$
   \item and $\{\delta_{16}^{11},\delta_{16}^{12},\delta_{16}^{13},\delta_{16}^{14},\delta_{16}^{15}\}$
  \end{itemize} 
    by their corresponding outputs.  
  \item And the $\mathsf{I}[0]=\delta_{4}^3$ can distinguish every two distinct initial states $\mathsf{s}$, $\mathsf{s}'\in \{\delta_{16}^0,\delta_{16}^{1},\delta_{16}^{2}\}$, 
  \item the $\mathsf{I}[0]=\delta_{4}^0$ can distinguish every two distinct initial states $\mathsf{s}$, $\mathsf{s}'\in \{\delta_{16}^3,\delta_{16}^{4},\delta_{16}^{5},\delta_{16}^{6}\}$,
  \item we can distinguish the sets of states $\{\delta_{16}^{8}\}$, $\{\delta_{16}^{9}\}$ and $\{\delta_{16}^7,\delta_{16}^{10}\}$ by the outputs of $\sigma (\delta_{4}^3, \delta_{16}^{8})$, $\sigma (\delta_{4}^3, \delta_{16}^{9})$, $\sigma (\delta_{4}^3, \delta_{16}^{7})$ and $\sigma (\delta_{4}^3, \delta_{16}^{10})$ after we input $\delta_{4}^3$,
  \begin{itemize}
  \item  and the $\mathsf{I}[0]=\delta_{4}^3$ can distinguish every two distinct initial states $\mathsf{s}$, $\mathsf{s}'\in \{\sigma (\delta_{4}^3, \delta_{16}^{7}),\sigma (\delta_{4}^3, \delta_{16}^{10})\}$,
  \end{itemize}
  \item we can distinguish the sets of states $\{\delta_{16}^{11},\delta_{16}^{15}\}$ and $\{\delta_{16}^{12},\delta_{16}^{13},\delta_{16}^{14}\}$ by the outputs of $\sigma (\delta_{4}^1, \delta_{16}^{11})$, $\sigma (\delta_{4}^1, \delta_{16}^{15})$, $\sigma (\delta_{4}^1, \delta_{16}^{12})$, $\sigma (\delta_{4}^1, \delta_{16}^{13})$ and $\sigma (\delta_{4}^1, \delta_{16}^{14})$ after we input $\delta_{4}^1$,
  \begin{itemize}
  \item  and the $\mathsf{I}[0]=\delta_{4}^0$ can distinguish every two distinct initial states $\mathsf{s}$, $\mathsf{s}'\in \{\sigma (\delta_{4}^1, \delta_{16}^{12}),\sigma (\delta_{4}^1, \delta_{16}^{13}), \sigma (\delta_{4}^1, \delta_{16}^{14})\}$,
   \item  the $\mathsf{I}[0]=\delta_{4}^2$ can distinguish every two distinct initial states $\mathsf{s}$, $\mathsf{s}'\in \{\sigma (\delta_{4}^1, \delta_{16}^{11}),\sigma (\delta_{4}^1, \delta_{16}^{15})\}$.
  \end{itemize}
\end{itemize} 
Thus we can determine its initial state by its outputs and inputs without resetting the initial state.
