...
We write $\mathbb{B}:=\{0,1\}$. %; $t=0,1,\ldots$ represents the discrete time. 
...
For instance if $\mathsf{s}(t)=\begin{vmatrix}0\\1\\0\\1\end{vmatrix}$, then $\mathsf{s}(t)$ can be represented by $\delta^{(1*2^2+1*2^0)}_{2^4}=\delta^5_{16}$. Therefore, the updating rules shown in Fig.\ref{fig:2} can be represented by following form.
 \begin{figure}[thpb]
      \centering
      \framebox{\parbox{3in}{
   ...
   With the abbreviation form, it would be easier for us to use this example to explain various concepts. And 
   \begin{itemize}
 % \item $\delta^i_{2^m}$ represents $\mathsf{i}(t)$ that $i$ is the decimal value of the vector;
 % \item $\delta^j_{2^n}$ represents $\mathsf{s}(t)$ that $j$ is the decimal value of the vector;
 % \item $\delta^w_{2^q}$ represents $\mathsf{o}(t)$ that $w$ is the decimal value of the vector;
 \item $\Delta_M$ represents the set of inputs $\{\delta^0_{2^m},\ldots,\delta^{{2^m}-1}_{2^m} \}$; 
 \item $\Delta_N$ represents the set of states $\{\delta^0_{2^n},\ldots,\delta^{({2^n}-1)}_{2^n} \}$; 
  \item $\Delta_Q$ represents the set of outputs $\{\delta^0_{2^q},\ldots,\delta^{{2^q}-1}_{2^q} \}$,
\end{itemize}
...
In this section, we introduce the four existing definitions of observability of \BCNs. %. In order to introduce four existing types of observability of \BCNs, 
...
Where $\Delta_N$, $\Delta_M$ and $\Delta_Q$ are three alphabets, $s_0\in \Delta_N$, $k\in \mathbb{Z}_+$ and $\infty$ is the infinite natural numbers. For all  $k\in \mathbb{N}^*$, \tl{where is $N^*$ defined?}
...
The first type of observability is that, a \BCN\ is called observable, if for every initial state \State$^{i}(0)$$\in \Delta_N$, there exists an input sequence $\mathsf{I}\in(\Delta_M)^k$ for some $k>0$, such that for all states $\mathsf{s}^{i}(0)\neq \mathsf{s}^{j}(0)\in \Delta_N$, $h(\mathsf{s}^{i}(0))=h(\mathsf{s}^{j}(0))$ implies $(HF)^k_{\mathsf{s}^{i}(0)}(\mathsf{I})\neq (HF)^k_{{\mathsf{s}^{j}(0)}}(\mathsf{I})$ \cite{cheng2009controllability}.
\end{definition}

Hence the first observability means that if a \BCN\ is observable, then every initial state \State$^{i}(0)$ of the \BCN\ can be distinguished from other types of initial state by an input sequence $\mathsf{I}\in(\Delta_M)^k$. However, we can only use the corresponding input sequence $\mathsf{I}$ of a type of initial state $\mathsf{s}^{i}(0)$ to check whether or not the initial state $\mathsf{s}(0)$ of this \BCN\ is the state $\mathsf{s}^{i}(0)$.
\begin{example}
...
\begin{example}
For example, for the \BCN\ mentioned in {\em Example \ref{exa:2}}, we have there is not any input sequence $\mathsf{I}\in(\Delta_M)^k$ which can determine the initial state of this \BCN. Because 
\begin{itemize}
  \item any $\mathsf{I}$ with prefix $\delta_{4}^0$ can not determine $\mathsf{s}(0)$ of this \BCN\ when $\mathsf{s}(0)=\delta_{16}^9, \delta_{16}^{10}, \delta_{16}^{11}$ or $\delta_{16}^{15}$;
  \item any $\mathsf{I}$ with prefix $\delta_{4}^1$ can not determine $\mathsf{s}(0)$ of this \BCN\ when $\mathsf{s}(0)=\delta_{16}^0, \delta_{16}^1, \delta_{16}^5, \delta_{16}^6, \delta_{16}^9$ or $\delta_{16}^{10}$;
  \item any $\mathsf{I}$ with prefix $\delta_{4}^2$ can not determine $\mathsf{s}(0)$ of this \BCN\ when $\mathsf{s}(0)=\delta_{16}^3, \delta_{16}^4, \delta_{16}^5$ or $\delta_{16}^6$;
  \item any $\mathsf{I}$ with prefix $\delta_{4}^3$ can not determine $\mathsf{s}(0)$ of this \BCN\ when $\mathsf{s}(0)=\delta_{16}^3, \delta_{16}^4, \delta_{16}^{14}$ or $\delta_{16}^{15}$.
\end{itemize} 
...
\begin{example}
For example, for the \BCN\ mentioned in {\em Example \ref{exa:2}}, we have there exists at least one sufficient long input sequence that can not determine the initial state of this \BCN. Any $\mathsf{I}$ with prefix $\delta_{4}^0$ can not determine $\mathsf{s}(0)$ of this \BCN\ when $\mathsf{s}(0)=\delta_{16}^9, \delta_{16}^{10}, \delta_{16}^{11}$ or $\delta_{16}^{15}$.  Therefore any input sequence which with the prefix $\delta_{4}^0$, they can not determine the initial state $\mathsf{s}(0)$ of the \BCN\ for every $\mathsf{s}(0)\in\Delta_N$.
