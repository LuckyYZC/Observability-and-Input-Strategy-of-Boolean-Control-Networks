...
Presenting the input sequence $\mathsf{i}(t)\ldots\mathsf{i}({t+k-1})$ by $\mathsf{I}[k]$, then the state sequence $\mathsf{s}(t+1)\ldots\mathsf{s}(t+k)$ and  output sequence $\mathsf{o}(t+1)\ldots\mathsf{o}(t+k)$ produced by $\mathsf{I}[k]$ from $\mathsf{s}(t)$ can be presented by $F^k_{\mathsf{s}(t)}(\mathsf{I}[k])$ and
 $H^k_{\mathsf{s}(t)}(\mathsf{I}[k])$, respectively. Thus, the observability, controllability and identifiability of \BCNs\ are defined as follows.
