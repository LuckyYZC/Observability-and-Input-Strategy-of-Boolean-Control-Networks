...
%,  $\mathbb{Z}$ to denote the set of integer 
\subsection{Boolean Control Networks}

We take the definition in ~\cite{Ideker2001A} in which a Boolean control network (\BCN)  is given a directed graph together with two  logical equations which define the updating rules for the  values of the nodes. 

\begin{definition}[Boolean Control Network] A \BCN\ is a tuple $\BB = (I,S,O, E, \sigma, \rho)$, where 
\begin{itemize}
\item $I$, $S$ and $O$ are three finite nonempty disjoint sets nodes (or vertices)
\begin{itemize}
...
\item The Boolean valued functions $\sigma: \mathbb{B}^m \mapsto \mathbb{B}^\ell$ and $\rho: \mathbb{B}^\ell\mapsto \mathbb{B}^n$ are functions from the pairs of $\ell$-dimensionand $m$-dimension vectors  of Boolean values to  the $\ell$-dimension vectors Boolean values and from the  $\ell$-dimension vectors to the  $n$-dimension vectors  of Boolean values, respectively. 
\item {\em Timed behavior}: each node $v\in I\cup S\cup O$ represents a Boolean variable, and we use 
 $i= (i_1,\ldots, i_\ell)$, $s= (s_1,\ldots, s_m)$ and $o= (o_1,\ldots, o_n)$ to denote the three variables Boolean vectors corresponding  to the input nodes, state nodes and output nodes. At any time $t\in \mathbb{T}$ during the execution 
 of $\BB$, each of the variables  $i$, $s$ and $o$  (thus every node of $\BB$ too) take a vector of Boolean values $i(t)$, $s(t)$, $o(t)$  in  $\mathbb{B}^\ell$, $\mathbb{B}^m$ and $\mathbb{B}^n$, respectively, such that the following equations are satisfied
\begin{equation}
\begin{split}
...
\label{equ:1}
\end{equation}
The above equations are also assumed to satisfy the conditions that 
\begin{enumerate}
\item the value of $s_k(t+1)$ in $s(t+1)$ is affected by the value  $i_j(t)$ of an input node $i_j \in I$ (or by the value $s_j(t)$ of a state node $s_j\in S$) at time $t$ only when $(i_j, s_k)\in E$ (or $(s_j, s_k)\in E$, respectively); and
\item the value $o_k(t)$ in $o(t)$ of an output node $o_k\in O$ is affected by the value $s_j(t)$ of a state node $s_j\in S$ only wen $(s_j,o_k)\in E$.
\end{enumerate}
\end{itemize}
 The two updating functions are also called the  {\bf updating rules}  of $\BB$. They define the value of the state nodes at any time by the values of the inout nodes and state nodes at the previous time point, and the the values of the output nodes by the value of the state nodes, respectively. 
\end{definition}

Furthermore, we use $\mathcal{I}_\BB=\{\delta_{2^\ell}^0,\ldots,\delta_{2^\ell}^{2^\ell -1}\}$, $\mathcal{S}_\BB=\{\delta_{2^m}^0,\ldots,\delta_{2^m}^{2^m -1}\}$ and $\mathcal{O}_\BB=\{\delta_{2^n}^0,\ldots,\delta_{2^n}^{2^n -1}\}$ to represent the sets of all possible inputs, states and outputs of $\BB$, respectively. And we will omit the subscript $\BB$ when there is no confusion. 
%we use $\mathsf{i}$ to represent a vector $(\mathsf{i}_1,\ldots \mathsf{i}_\ell)$ of input values, $\mathsf{s}$ a vector $(\mathsf{s}_1,\ldots \mathsf{s}_m)$ of state values, and $\mathsf{o}$ a vector $(\mathsf{o}_1,\ldots \mathsf{o}_n)$ of output values, which called an {\em input}, a {\em state} and an {\em output} for simplicity.  And at any time $t\in \mathbb{T}$ during the execution of $\BB$,  %Obviously, $\mathcal{I}_\BB$, and $\mathcal{S}_\BB$ and $\mathcal{O}_\BB$ are finite with $2^\ell$, $2^m$ and $2^n$ elements, respectively. %that is $\mathcal{I}_\BB=\{\delta_{2^\ell}^0,\ldots,\delta_{2^\ell}^{2^\ell -1}\}$, $\mathcal{S}_\BB=\{\delta_{2^m}^0,\ldots,\delta_{2^m}^{2^m -1}\}$ and $\mathcal{O}_\BB=\{\delta_{2^n}^0,\ldots,\delta_{2^n}^{2^n -1}\}$. 
%That we use 
%\begin{itemize}
%  \item $\delta^x_{2^m}$ represents the abbreviation form of $\mathsf{i}$, where $i=\sum_{x=1}^m \mathsf{i}_{x}\times 2^{m-x}$;
 % \item $\delta^y_{2^n}$ represents the abbreviation form of $\mathsf{s}$, where $j=\sum_{x=1}^n \mathsf{s}_{x}\times 2^{n-x}$;
%  \item $\delta^z_{2^q}$ represents the abbreviation form of $\mathsf{o}$, where  $w=\sum_{x=1}^q \mathsf{o}_{x}\times 2^{q-x}$.
%\end{itemize}
...
$\mathsf{o}_1(t)=\mathsf{s}_1(t)\vee {({\mathsf{s}_2}(t)\wedge { \mathsf{s}_3}(t)\wedge {\mathsf{s}_4}(t))}$. The set of all possible inputs $\mathcal{I}_\BB=\{\delta_{4}^0,\ldots,\delta_{4}^{3}\}$, the set of all states $\mathcal{S}_\BB=\{\delta_{16}^0,\ldots,\delta_{16}^{15}\}$ and the set of all outputs $\mathcal{O}_\BB=\{\delta_{4}^0,\ldots,\delta_{4}^{3}\}$.
 \begin{figure}[thpb]
	\centering
	\framebox{\parbox{3in}{
...
To introduce the observability, controllability and identifiability of \BCNs, we first define two functions to present the relations among the input sequence, state squence and output sequence~\cite{Zhang2016Observability}:
\begin{equation}
\begin{split}
F^k_{\mathsf{s}(t)}:& \mathcal{I}^k\mapsto\mathcal{S}^k,\\
F^k_{\mathsf{s}(t)}&(\mathsf{i}(t)\ldots\mathsf{i}({t+k-1}))\\
%&= \mathsf{s}(t+1)\ldots\mathsf{s}(t+k)\\
=&\sigma(\mathsf{i}(t),\mathsf{s}(t))\ldots\sigma(\mathsf{i}(t+k-1),\mathsf{s}(t+k-1))
\end{split}
\end{equation}
\begin{equation}
\begin{split}
H^k_{\mathsf{s}(t)}:& \mathcal{I}^k\mapsto\mathcal{O}^k,\\
 H^k_{\mathsf{s}(t)}&(\mathsf{i}(t)\ldots\mathsf{i}(t+k-1)) \\%\mapsto \mathsf{o}(t+1)\ldots\mathsf{o}(t+k)
 =&\rho(\sigma(\mathsf{i}(t),\mathsf{s}(t)))\ldots\rho(\sigma(\mathsf{i}(t+k-1),\mathsf{s}(t+k-1)))\\
\end{split}
\label{equ:6}
\end{equation}
%\tl{but you have defined $\Delta_N$, $\Delta_M$ and $\Delta_Q$ just now?!}
where $k>0$ presents the length of input sequence.

Such that, for any time $t\in  \mathbb{T}$ and any $k>0$, using $\mathsf{I}[k]$ to presents the input sequence $\mathsf{i}(t)\ldots\mathsf{i}({t+k-1})$, the state sequence $\mathsf{s}(t+1)\ldots\mathsf{s}(t+k)$ and  output sequence $\mathsf{o}(t+1)\ldots\mathsf{o}(t+k)$ produced by $\mathsf{I}[k]$ from $\mathsf{s}(t)$ can be presented by $F^k_{\mathsf{s}(t)}(\mathsf{I}[k])$ and
 $H^k_{\mathsf{s}(t)}(\mathsf{I}[k])$, respectively. 
...
\begin{itemize}
  \item The {\bf Type-I} observability is that, a \BCN\ is observable if for every initial state \State$(0)\in \mathcal{S}$, there exists an input sequence $\mathsf{I}[k]\in\mathcal{I}^k$ for some $k>0$, such that for all states $\mathsf{s}'(0)\neq \mathsf{s}(0)$, $\rho(\mathsf{s}'(0))=\rho(\mathsf{s}(0))$ implies $H^k_{\mathsf{s}'(0)}(\mathsf{I}[k])\neq H^k_{{\mathsf{s}(0)}}(\mathsf{I}[k])$.
