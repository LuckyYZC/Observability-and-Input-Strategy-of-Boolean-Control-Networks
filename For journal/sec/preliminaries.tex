...
\end{itemize}	
Each node at any time takes a Boolean value in $\mathbb{B}$, and $\BB$'s {\bf updating rules}  $\sigma$ and $\rho$ are Boolean valued functions which define relations among the values of the nodes such that for any time $t\in  \mathbb{T}$
\begin{equation}
\begin{split}
...
\label{equ:1}
\end{equation}
where $\mathsf{i}(t)$, $\mathsf{s}(t)$, and $\mathsf{o}(t)$ represent the vectors of values $(\mathsf{i}_1(t),\ldots \mathsf{i}_\ell(t))$, $(\mathsf{s}_1(t),\ldots \mathsf{s}_m(t))$ and $(\mathsf{o}_1(t),\ldots \mathsf{o}_n(t))$, respectively.
 \end{definition}

 
It is noted that there are two sets of edges in $E$, that is they are the subsets of $(I\cup S)\times S$ and $S\times O$, respectively. An edge $(v,v')$ represents that the value of node $v'$ is directly affected by the value of $v$, and the updating rules reflect how a node is affected by the other nodes. 
