...
\begin{itemize}
  \item The {\bf Type-I} observability is that, a \BCN\ is observable if for every initial state $\mathsf{s}\in \mathcal{S}$, there exists an input sequence $\mathsf{I}[t]\in\mathcal{I}^{[t]}$ for some $t\ge0$, such that for all states $\mathsf{s}'$, $H^{[0,t]}(\mathsf{s}'(0),\mathsf{I}[t])\neq H^{[0,t]}(\mathsf{s}(0), \mathsf{I}[t])$ if $\mathsf{s}\ne\mathsf{s}'$.
  \item The  {\bf Type-II} observability is that, a \BCN\ is observable if for every two distinct initial states $\mathsf{s}$ and $\mathsf{s}' \in \mathcal{S}$, there is an input sequence $\mathsf{I}[t]\in\mathcal{I}^{[t]}$ for some $t\ge0$, such that $H^{[0,t]}(\mathsf{s}',\mathsf{I}[t])\neq H^{[0,t]}(\mathsf{s}, \mathsf{I}[t])$.
  \item The {\bf Type-III} observability is that, a \BCN\ is observable if there exists an input sequence $\mathsf{I}[t]\in\mathcal{I}^{[t]}$ for some $t\ge0$, such that for any two distinct states $\mathsf{s}(0)$, $\mathsf{s}'(0) \in \mathcal{S}$, $H^{[0,t]}(\mathsf{s}'(0),\mathsf{I}[t])\neq H^{[0,t]}(\mathsf{s}(0), \mathsf{I}[t])$.
...
\begin{definition}[Controllability~\cite{cheng2009controllability}]
	A \BCN\ is controllable if for any two distinct states $\mathsf{s}$, $\mathsf{s}' \in \mathcal{S}$, there is an input sequence $\mathsf{I}[t]\in\mathcal{I}^{[t]}$ for some $t>0$, such that the last state $\mathsf{s}(t+1)$ of $F^{[0,t]}(\mathsf{s}, \mathsf{I}[t])=\mathsf{s}(0) \ldots\, \mathsf{s}(t+1)$ is $\mathsf{s}'$.
\end{definition}
...
\begin{definition}[Identifiability~\cite{Cheng2011Identification}]%
	Assume that we have a Boolean control network, the identification problem involves finding the functions % \BCN\ is identifiable if for every initial state \State$(0)\in \mathcal{S}$, there exists an input sequence $\mathsf{I}[t]\in\mathcal{I}^{[t]}$ for some $t>0$, such that the updating rules
	\begin{equation*}
    		\begin{split}
		\mathsf{s}(t+1)=&\sigma(\mathsf{i}(t),\mathsf{s}(t))\\
		\mathsf{o}(t)=&\rho(\mathsf{s}(t))
		\end{split}
	\end{equation*}
	via certain input-output data $\mathsf{i}(0)\ldots \mathsf{i}(t)$ and $\mathsf{o}(0)\ldots \mathsf{o}(t)$. Then, a \BCN\ is identifiable if $\sigma$ and $\rho$ can be uniquely determined by using designed inputs $\mathsf{i}(0)\ldots \mathsf{i}(t)$. 
\end{definition}
