...
\begin{abstract}
Determining initial state of Boolean control networks (\BCNs)  is import in many applications.  The existing 
...
the input sequence is given before studying the \BCN's initial state and the input sequence  remains unchanged during the process of determining.
 ...
 \ly{Compared with existing four observabilities, we provide an online observability which is more powerful on  determines the initial state.}
